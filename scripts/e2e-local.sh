...
SKIP_DEPS_INSTALL="${SKIP_DEPS_INSTALL:-false}"
...
install_dependencies() {
    if [[ "$SKIP_DEPS_INSTALL" == "true" ]]; then
        log_info "Skipping dependencies installation (SKIP_DEPS_INSTALL=true)"
        return
    fi
...
build_and_load_operator() {
    ...
    docker save localhost/kpodautoscaler:e2e-test -o kpodautoscaler.tar
...
run_e2e_tests() {
    ...
    export KIND_CLUSTER="${KIND_CLUSTER_NAME}"
    export KUBECONFIG="${HOME}/.kube/config"
    export TEST_OPERATOR_IMAGE="localhost/kpodautoscaler:e2e-test"
    
    # Run tests
    log_info "Starting E2E tests..."
    if go test ./test/e2e/... -v -ginkgo.v -timeout 30m; then
...
collect_artifacts() {
    ...
    local artifacts_dir="${PROJECT_ROOT}/e2e-artifacts"
...
# Main execution
main() {
    log_section "KPodAutoscaler E2E Test Runner"
...
usage() {
    ...
    echo "  SKIP_DEPS_INSTALL       Skip dependencies installation if set to 'true'"
